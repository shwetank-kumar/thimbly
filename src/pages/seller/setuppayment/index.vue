...
export default {
  ...
  methods: {
    async setup() {
      ...
      const account_holder_name = this.$store.state.stripeSetup.individual.first_name + ' ' 
      ...
      const individual= { first_name: this.$store.state.stripeSetup.individual.first_name,
      ...
      const stripe_account = await axios.post(stripe_setup_url, { type, country, business_profile, business_type, 
